...
# load data
df = pd.read_csv('data/locations_sorted.tsv', sep='\t')

# change long to lng
df = df.rename(columns={'long': 'lng'})

# change country codes to country names
df['country'] = df['country'].replace({'GB': 'Great Britain', 'CH': 'Switzerland'})

print(df)

fig = px.scatter_geo(df, lat='lat', lon='lng', hover_name='name', color='country', projection='natural earth',
                     size_max=10, opacity=0.5, title='BSCC Locations',
                     labels={'name': 'Name'},
                     hover_data={'Unnamed: 0': False, 'lat': False, 'lng': False, 'id': False, 'country': False, 'count': True})


fig.show()

# interactive map bscc atlas locations
